...
> Rule file for Cline (place in `.clinerules/mybd-cryfs.md` or import into
> `.clinerules`). Trigger keywords: "encrypt my credentials", "cryfs vault",
> "mybd", "protect passwords folder", "fuse encryption setup",
> "secrets encrypted-at-rest". Apply when the user wants to set up, migrate,
> operate, or troubleshoot a cryfs-encrypted secrets folder, or when an agent
> needs safe open/close discipline for such a vault.
